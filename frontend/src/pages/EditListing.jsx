...
import { PencilLine, ArrowLeft, Trash2, Upload, Sparkles } from 'lucide-react';
...
export const EditListing = () => {
  ...
  const fetchListingDetails = async () => {
    try {
      ...
      if (res.data && res.data.listing) {
        ...
        setType(listing.type || 'Villa');

        // Parse images array or default to single image object
  ...
  const handleGenerateDescription = async () => {
    if (!title) {
      toast.info('Please specify a Nest Title first.');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (existingImages.length === 0 && newImageFiles.length === 0) {
      toast.error('Listing must have at least one stay image!');
      return;
    }
    ...
    // Send remaining existing images as JSON string
  ...
  // Combined list of previews to locate the cover index correctly
  const allImagesPreviews = [
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.03)] sm:p-10">
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 pb-5 mb-6">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-rose-50 text-brand-rose">
            <PencilLine className="h-6 w-6 stroke-[2.2]" />
          </div>
          <div>
            ...
            <p className="mt-1.5 text-xs font-semibold text-slate-400">
              Manage upload arrays and leverage Gemini text engines
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Images drag and drop and previews */}
          <div className="flex flex-col gap-1.5">
            ...
            >
              ...
              <span className="text-xs font-bold text-slate-600">Drag & drop new images to append</span>
              ...
              <input
                ...
                id="images-uploader-edit"
              />
            </div>

            {/* Preview gallery showing existing & new files */}
            {allImagesPreviews.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                {allImagesPreviews.map((url, idx) => {
          ...
          <div className="pt-4 border-t border-slate-100 flex justify-end">
            ...
            >
              {submitting ? 'Updating Nest...' : 'Update Nest'}
            </button>
          </div>
